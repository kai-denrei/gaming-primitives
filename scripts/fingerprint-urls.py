...
def walk_target(target, skip_dirs):
    for root, dirs, files in os.walk(target):
        ...
        # Skip cb-shapes; skip mini-game PWAs and the template — those have
        # their own __VERSION__ / CACHE_VERSION pipeline and must not be
        # touched by the hub's fingerprint pass.
        if rel.startswith("public/cb-shapes") or "/public/cb-shapes" in rel:
            continue
        if rel == "public/g" or rel.startswith("public/g/"):
            continue
        if rel == "templates" or rel.startswith("templates/"):
            continue
        for f in files:
            yield Path(root) / f
