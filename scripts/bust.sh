...
# Token derives from git SHA (8 hex chars) so badge + footer + asset URLs
# share one identity. When the tree is dirty, prepend a 'd' marker so the
# favicon cell shifts and you can see at a glance that you're not on a clean
# build. Falls back to /dev/urandom outside a git checkout.
if command -v git >/dev/null 2>&1 && git rev-parse --git-dir >/dev/null 2>&1; then
  TOKEN=$(git rev-parse --short=8 HEAD 2>/dev/null || true)
  if [[ -n "$TOKEN" ]] && [[ -n "$(git status --porcelain 2>/dev/null)" ]]; then
    TOKEN="d${TOKEN:0:7}"
  fi
fi
if [[ -z "${TOKEN:-}" ]]; then
