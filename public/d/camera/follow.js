// Camera: follow — camera center tracks the player position directly (lerp at follow_speed=1 is instant).
...
export function init(ctx, params, env) {
  ...
  function tick(dt) {
    ...
    // follow: follow_speed=1 → instant, otherwise lerp proportional to follow_speed
    const k = Math.min(1, state.follow);
    state.camX += (p.x - state.camX) * k;
    state.camY += (p.y - state.camY) * k;
    const c = clampCam(state.camX, state.camY);
    state.camX = c.x; state.camY = c.y;

    drawScene(state.camX, state.camY);

    // ideal indicator: where camera would be without clamp/smoothing (= player pos)
...
export function applyParams(state, params) {
  ...
  state.follow = params.follow_speed;
}
