// Camera: dead-zone — camera only shifts when the player exits a centered tolerance rectangle.
...
export function init(ctx, params, env) {
  const state = {
    ...
    dzw: params.dead_zone_w,
    dzh: params.dead_zone_h,
    landmarks: landmarks(),
    camX: 400,
    camY: WORLD_H / 2,
  };

  function drawScene(camX, camY, dzw, dzh) {
    ...
    // deadzone outline (centered on camera)
    ctx.strokeStyle = '#888';
    ctx.lineWidth = 1;
    ctx.setLineDash([4, 4]);
    ctx.strokeRect(wx(camX - dzw / 2), wy(camY - dzh / 2), dzw * SCALE, dzh * SCALE);
    ctx.setLineDash([]);
  }
  ...
  function tick(dt) {
    ...
    // dead-zone push: camera only shifts to keep player inside the deadzone
    const dx = p.x - state.camX, dy = p.y - state.camY;
    const halfW = state.dzw / 2, halfH = state.dzh / 2;
    if (dx >  halfW) state.camX += dx - halfW;
    if (dx < -halfW) state.camX += dx + halfW;
    if (dy >  halfH) state.camY += dy - halfH;
    if (dy < -halfH) state.camY += dy + halfH;
    const c = clampCam(state.camX, state.camY);
    state.camX = c.x; state.camY = c.y;

    drawScene(state.camX, state.camY, state.dzw, state.dzh);
...
export function applyParams(state, params) {
  ...
  state.dzw = params.dead_zone_w;
  state.dzh = params.dead_zone_h;
}
